...
test_violation {
	...
	test.assert_fail(finding) with input as rule_input("--protect-kernel-defaults=false") with data.benchmark_data_adapter as data_adapter
	test.assert_fail(finding) with input as rule_input_with_external("--protect-kernel-defaults=false", create_process_config(true)) with data.benchmark_data_adapter as data_adapter
}

test_pass {
	test.assert_pass(finding) with input as rule_input("--protect-kernel-defaults=true") with data.benchmark_data_adapter as data_adapter
	test.assert_pass(finding) with input as rule_input_with_external("--protect-kernel-defaults=true", create_process_config(false)) with data.benchmark_data_adapter as data_adapter
	test.assert_pass(finding) with input as rule_input_with_external("--protect-kernel-defaults=true", create_process_config(false)) with data.benchmark_data_adapter as data_adapter
