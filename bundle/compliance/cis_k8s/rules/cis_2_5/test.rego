...
test_violation {
	...
	test.assert_fail(finding) with input as rule_input("--peer-client-cert-auth=false") with data.benchmark_data_adapter as data_adapter
}

test_pass {
	test.assert_pass(finding) with input as rule_input("--peer-client-cert-auth=true") with data.benchmark_data_adapter as data_adapter
}
