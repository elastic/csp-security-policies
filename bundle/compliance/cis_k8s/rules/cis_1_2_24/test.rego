...
test_violation {
	test.assert_fail(finding) with input as rule_input("--service-account-lookup=false") with data.benchmark_data_adapter as data_adapter
}

test_pass {
	test.assert_pass(finding) with input as rule_input("") with data.benchmark_data_adapter as data_adapter
	test.assert_pass(finding) with input as rule_input("--service-account-lookup=true") with data.benchmark_data_adapter as data_adapter
}
