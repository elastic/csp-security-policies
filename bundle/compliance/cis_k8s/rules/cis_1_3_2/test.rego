...
test_violation {
	...
	test.assert_fail(finding) with input as rule_input("--profiling=true") with data.benchmark_data_adapter as data_adapter
}

test_pass {
	test.assert_pass(finding) with input as rule_input("--profiling=false") with data.benchmark_data_adapter as data_adapter
}
