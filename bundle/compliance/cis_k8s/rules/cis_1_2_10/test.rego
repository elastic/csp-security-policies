...
test_violation {
	...
	test.assert_fail(finding) with input as rule_input("--enable-admission-plugins=NamespaceLifecycle,AlwaysDeny") with data.benchmark_data_adapter as data_adapter
}

test_pass {
	test.assert_pass(finding) with input as rule_input("--enable-admission-plugins=EventRateLimit") with data.benchmark_data_adapter as data_adapter
	test.assert_pass(finding) with input as rule_input("--enable-admission-plugins=NamespaceLifecycle,EventRateLimit,NodeRestriction,AlwaysPullImages") with data.benchmark_data_adapter as data_adapter
}
