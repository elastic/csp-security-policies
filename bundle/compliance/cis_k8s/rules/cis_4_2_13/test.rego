...
test_violation {
	...
	test.assert_fail(finding) with input as rule_input_with_external("", create_process_config(["SOME_CIPHER"])) with data.benchmark_data_adapter as data_adapter
	test.assert_fail(finding) with input as rule_input_with_external("", create_process_config(["TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,SOME_CIPHER", "SOME_CIPHER"])) with data.benchmark_data_adapter as data_adapter
}
...
test_pass {
	...
	test.assert_pass(finding) with input as rule_input_with_external("", create_process_config(["TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256"])) with data.benchmark_data_adapter as data_adapter
	test.assert_pass(finding) with input as rule_input_with_external("", create_process_config(["TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256", "TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384"])) with data.benchmark_data_adapter as data_adapter
}
